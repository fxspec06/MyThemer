enyo.kind({
	...
	components: [
		...
		//@* swap as necessary
			{name: '_box', layoutKind: 'FittableRowsLayout', classes: 'enyo-fit', fit: true },
	...
	runBackground: function() {
		this.log();
		var iterate = ['timeline','mentions','messages'],
			exclude = ['filters'];
...
//			console.log('swapping..', this.currentBox, newBox, enyo.clone(this), enyo.clone(_saveData));
	...
	buildHeaders: function(chosen) {
		this.$.toolbar.setCloseable(true);
		switch (chosen.toLowerCase()) {
			case 'filters': case 'favorites': case 'retweets': case 'trends':
				this.$.toolbar.setHeader(chosen);
				break;
			case 'lists':
			...
			default:
				this.$.toolbar.setCloseable(false);
				this.$.toolbar.setHeader(chosen);
				break;
		}
		this.$.toolbar.render();
...
//				AppUI.viewUser(account.username, account.service, account.account_id);
	...
	//@* signals || AppUI
	search: function(inQuery, inAccountId) {
		...
		if (typeof exist === 'number') {
			...
			this.deleteColumn();
			this.search(inQuery, inAccountId);
			return;
		}
		
		this.createColumn({ type: 'search', accounts: [inAccountId], query: inQuery });
		this.loadColumn('search');

enyo.kind({
			...
			function loadData(account_id) {
				...
				switch (self.info.type) {
					...
					case SPAZ_COLUMN_SEARCH:
						self.loadStarted();
						self.twit.search(self.info.query, since_id, 50, _n, _n, _n,
							function(data) {self.loadFinished(data, opts, account_id)},
						self.loadFailed);
						break;
					...
					case SPAZ_COLUMN_RETWEETS:
						...
						self.twit[rtname](_n,_n,_n,_n,function(data) {
							self.loadFinished(data, opts, account_id)}, self.loadFailed);
						break;
					//@* extended
	...
	setLastRead: function() {
		...
		if (this.tweets.length > 0) {
			var newest_item = _.max(this.tweets, function(_t) {return _t.publish_date}) || this.tweets[0]._orig.created_at;
